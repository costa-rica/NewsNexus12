...
type WorkerNodeJobStatusPanelProps = {
  endpointName: string;
...
function getStatusClasses(status: WorkerNodeJobRecord["status"]): string {
  switch (status) {
    case "completed":
...
export function WorkerNodeJobStatusPanel({
  endpointName,
  refreshSignal = 0,
  title,
}: WorkerNodeJobStatusPanelProps) {
  const { token } = useAppSelector((state) => state.user);
  const [job, setJob] = useState<WorkerNodeJobRecord | null>(null);
  ...
  const fetchLatestJob = async (showErrorModal = true) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/automations/worker-node/latest-job?endpointName=${encodeURIComponent(endpointName)}`,
        {
      ...
      const result = (await response.json()) as { job: WorkerNodeJobRecord | null };
      setJob(result.job);
    } catch (error) {
      if (showErrorModal) {
        setAlertModal({
  ...
  const handleCancelJob = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/automations/worker-node/cancel-job/${encodeURIComponent(job.jobId)}`,
        {
          headers: {
        ...
        <div className="mb-4 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
              Latest worker-node job for this workflow.
            </p>
          </div>
