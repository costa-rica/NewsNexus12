...
version: v03
supersedes: 20260701_api_pagination_oom_fix_plan_v02.md
---

# API Pagination OOM Fix — Plan v03

## Changelog (v02 → v03)

This revision responds to `docs/20260701_api_pagination_oom_fix_plan_v02_assessment_codex.md`.
Codex confirmed v02's main SQL filter mapping is correct and raised one qualifying concern: the
testing section introduced a **contradictory expectation** for `POST /articles/with-ratings`
approval filtering. v02 said the false-approval fixture "resolves the opposite way" between
`POST /articles` and `POST /articles/with-ratings`, but both endpoints exclude only **true**
approvals, so both must **keep** an article whose only approval row has `isApproved = false`
under `returnOnlyIsNotApproved`. Only the **null-relevance** fixture actually distinguishes the
two endpoints.

- **Fix (testing section only).** The filter-parity fixtures are rewritten so that:
  - the **false-approval fixture** (`ArticleApproveds.isApproved = false`) is **kept by both**
    `POST /articles` and `POST /articles/with-ratings` under `returnOnlyIsNotApproved` — it does
    **not** distinguish the endpoints, because each excludes only true approvals;
  - the **null-relevance fixture** (`ArticleIsRelevants.isRelevant IS NULL`) is the **only**
    fixture that distinguishes them: `POST /articles` **excludes** it under
    `returnOnlyIsRelevant` (any relevance row, including null, disqualifies), while
    `POST /articles/with-ratings` **keeps** it (only a **non-null** `isRelevant` row
    disqualifies).

No SQL mapping, endpoint design, portal change, or scope decision from v02 is altered; the
correction is confined to the fixture expectations in [[#Testing and verification]]. The v02
changelog is retained below for continuity.

## Changelog (v01 → v02, retained)

This revision responded to `docs/20260701_api_pagination_oom_fix_plan_v01_assessment_codex.md`.
Codex raised two qualifying concerns, both about the SQL translation of `POST /articles`
filters silently changing which articles the add/delete list returns. Both were fixed in v02;
the rest of v01's intent was preserved. Fixes were verified against the current code
...
- **Concern 1 — `returnOnlyIsNotApproved` for `POST /articles`.** The route builds its approval
  ...
  rows are non-approved — a behavior change. **Corrected** to
...
- **Concern 2 — `returnOnlyIsRelevant` for `POST /articles`.** The route builds its relevance
  ...
  **Corrected** to `NOT EXISTS (SELECT 1 FROM "ArticleIsRelevants" air WHERE
  air."articleId" = a.id)`.
...
- Filter parity **per endpoint**: the SQL-filtered results match the previous
  JavaScript-filter semantics for every approved/relevant combination.

  Because `POST /articles` and `POST /articles/with-ratings` differ **only** in relevance
  semantics, the fixtures must be chosen carefully. There are two distinct fixtures, and they
  play different roles:

  - **False-approval fixture — shared, does *not* distinguish the endpoints.** An article whose
    only `ArticleApproveds` row has `isApproved = false`. Under `returnOnlyIsNotApproved` this
    article must be **kept by both** `POST /articles` **and** `POST /articles/with-ratings`,
    because each endpoint excludes only **true** approvals
    (`NOT EXISTS (... aa."isApproved" = true)`). Assert the same "kept" expectation for both
    endpoints — do **not** expect them to diverge on this fixture. (This corrects the v02
    "resolve the opposite way" wording flagged by Codex.)

  - **Null-relevance fixture — the *only* fixture that distinguishes the endpoints.** An article
    whose only `ArticleIsRelevants` row has `isRelevant IS NULL`. Under `returnOnlyIsRelevant`:
    - `POST /articles` must **exclude** this article — its relevance map has no `isRelevant`
      predicate, so **any** relevance row (including null) disqualifies
      (`NOT EXISTS (SELECT 1 FROM "ArticleIsRelevants" air WHERE air."articleId" = a.id)`).
    - `POST /articles/with-ratings` must **keep** this article — it excludes only rows with a
      **non-null** `isRelevant`
      (`NOT EXISTS (... AND air."isRelevant" IS NOT NULL)`), so a null-relevance row does not
      disqualify.

  For completeness also assert the straightforward cases per endpoint: an article with a
  **true** approval row is excluded under `returnOnlyIsNotApproved` by both endpoints; an
  article with a **non-null** relevant row is handled per each endpoint's `returnOnlyIsRelevant`
  rule; and an article with no approval/relevance rows behaves consistently with the current
  JavaScript filters.
