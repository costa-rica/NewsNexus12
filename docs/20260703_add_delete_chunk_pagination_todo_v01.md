...
- [ ] `api/src/modules/queriesSql.ts`: add pure exported
...
- [ ] Add `sqlQueryArticleIdsForArticlesRoute(filters, cursor, limit)`:
...
- [ ] Add `sqlQueryCountArticlesForArticlesRoute(filters)`: `SELECT COUNT(*)` over
  `"Articles" a` + builder WHERE (no cursor). Returns a number.
- [ ] Add optional `articleIds` bounding to the four shared helpers — backward compatible;
...
- [ ] Export `buildArticlesListWhereClause`, `sqlQueryArticleIdsForArticlesRoute`, and
  `sqlQueryCountArticlesForArticlesRoute` from the export block (~line 940).
- [ ] New unit tests in `api/tests/modules/` following the fix02 WHERE-builder test pattern,
...
- [ ] Unit tests for the helper bounding (mock `sequelize.query` per existing module-test
...
- [ ] Rework `POST /articles` (~lines 60–208):
...
- [ ] Do not modify `GET /articles/summary-statistics` (~lines 621–686) — it continues
  calling the helpers with no arguments.
- [ ] Extend `api/tests/articles/articles.routes.test.ts` (helpers mocked — orchestration
...
- [ ] Copy `ARTICLE_LIMIT_OPTIONS` verbatim from the review page (lines ~61–68) as a
...
- [ ] Add chunk state, mirroring the review page (~lines 84–90):
...
- [ ] Rework `fetchArticlesArray` (currently ~lines 85–120) to
...
- [ ] Add `initialFilters` + `hasFilterChanges`, replicating the review page (~lines
  ~100–140), covering the four filter fields plus `limit` (`?? 20000` fallback on both
  sides of the comparison).
- [ ] Add `handleRefreshWithFilters`, mirroring the review page: `setInitialFilters` from
...
- [ ] Add Prev/Next handlers, mirroring the review page (~lines 725–760): Next (enabled when
  ...
  index.
- [ ] Insert the Filter Articles card above the `TableReviewArticles` usage, copied from the
...
- [ ] Leave the rest of the page unchanged (add form, `handleSelectArticleFromTable`, delete
...
- [ ] Full api test suite: `cd api && npm test`.
