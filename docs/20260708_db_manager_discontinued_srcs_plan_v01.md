...
# Db manager delete discontinued-source articles plan v01

## Purpose

1. Add a db-manager CLI operation that deletes articles found by the discontinued aggregator sources **NewsAPI**, **GNews**, and **NewsData.IO**.
...
1. New flag: `--delete_articles_discontinued_sources`, with an optional positive-integer limit:
   - `--delete_articles_discontinued_sources` — delete all eligible articles,
   - `--delete_articles_discontinued_sources <n>` / `--delete_articles_discontinued_sources=<n>` — cap deletions for a cautious first pass,
   - `--delete_articles_discontinued_sources --dry_run` — read-only preview.
2. The operator's requested spelling was `--delete_articles_from_discontinuted_sources`; this plan corrects the typo ("discontinuted") and drops `from_` so the name follows the established `--delete_articles_<qualifier>` pattern (`_trim`, `_no_state`). If the operator prefers the literal requested name, only the constant in `cli.ts` changes.
...
1. A module-level constant lists the discontinued source names exactly as the api package writes them: `["NewsAPI", "GNews", "NewsData.IO"]` (verified in `api/src/modules/newsOrgs/requestsNewsApi.ts`, `requestsGNews.ts`, `requestsNewsDataIo.ts` — all query `nameOfOrg` with these exact strings).
...
3. that source's `nameOfOrg` is one of the three discontinued names.
...
## Module: `src/modules/deleteArticlesDiscontinuedSources.ts`

1. Mirrors `deleteArticlesNoState.ts` exactly in shape — two exports:
   - `getDiscontinuedSourcesDeletionPreview(limit?)` — pure read. One raw SQL query (consistent with the no-state module) that inner-joins `Articles` → `EntityWhoFoundArticles` → `NewsArticleAggregatorSources`, filters `nameOfOrg IN (...)`, and returns per-row: `articleId`, `title`, `publishedDate`, `sourceName` (`nameOfOrg`), plus the four `EXISTS(...)` protection columns. Returns a summary object:
   ...
   - `deleteDiscontinuedSourcesArticles({ dryRun, limit })` — calls the preview, logs the summary, stops when `dryRun` is true or nothing is eligible; otherwise deletes `eligible` ids with `Article.destroy({ where: { id: { [Op.in]: batchIds } } })` in batches of 5000 with progress logging — the same deletion mechanics production already runs.
...
1. `src/modules/cli.ts`: new branch for `--delete_articles_discontinued_sources` with optional positive-integer value (inline `=n` or next-arg), exactly mirroring the `--delete_articles_no_state` branch. New `CliOptions` fields: `deleteArticlesDiscontinuedSources?: boolean`, `deleteArticlesDiscontinuedSourcesLimit?: number` in `src/types/cli.ts`.
2. `src/index.ts` dry-run gating: `--dry_run` currently composes with `--zip_file` XOR `--delete_articles_no_state`. Extend to allow exactly one dry-runnable target among `--zip_file`, `--delete_articles_no_state`, `--delete_articles_discontinued_sources`; combining two or more with `--dry_run` is an error, and `--dry_run` alone remains an error. Error messages updated to name all three.
3. Execution order in `src/index.ts` becomes: backup, import, trim, delete, no-state delete, **discontinued-sources delete**, status.
...
2. Module tests (new `tests/modules/deleteArticlesDiscontinuedSources.test.ts`, mocked models like the existing suites):
...
1. Dry run: `npm start -- --delete_articles_discontinued_sources --dry_run`
2. Inspect total/per-source counts, per-protection exclusions, and samples; confirm no missing-source warnings.
3. Limited execute: `npm start -- --delete_articles_discontinued_sources 1000`
4. Re-run the dry run and confirm counts dropped as expected.
5. Full execute: `npm start -- --delete_articles_discontinued_sources`
...
1. Add `--delete_articles_discontinued_sources [n]` to `src/modules/cli.ts` and `src/types/cli.ts`; register in `KNOWN_FLAGS`.
2. Extend `src/index.ts` dry-run gating and execution order.
3. Add `src/modules/deleteArticlesDiscontinuedSources.ts` with `getDiscontinuedSourcesDeletionPreview(limit?)` and `deleteDiscontinuedSourcesArticles({ dryRun, limit })`: inner-join candidate query against the three exact source names, missing-source warnings, always-on precedence-counted protections, batched `Article.destroy()`.
