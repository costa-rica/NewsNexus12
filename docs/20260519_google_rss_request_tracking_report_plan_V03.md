# Plan: Google RSS Query Tracking Sheet in Weekly Orchestrator Report — V03

Status: DRAFT — V03, incorporates V02 Codex review feedback
Supersedes: [20260519_google_rss_request_tracking_report_plan_V02.md](20260519_google_rss_request_tracking_report_plan_V02.md)
...
V02 was reviewed by Codex ([assessment](20260519_google_rss_request_tracking_report_plan_V02_assessment_codex.md)). Two issues were raised. Both **accepted**:

1. **`target_reached` and late `canceled` must NOT overwrite the active row.** Both fire after `storeRequestAndArticles` has already succeeded — the row that triggered the break legitimately succeeded and must keep its `success / savedCount` outcome. `target_reached` and "canceled after a successful query" are run-ending reasons (already captured in `endingReason` / `endingMessage`), not per-row outcomes.
2. **Non-503 RSS errors must still call `storeRequestAndArticles`.** Today's code persists a `NewsApiRequest` row with `status='error'` and empty items for non-503 errors. That history record is what `wasRequestMadeRecently` uses to enforce the repeat window for failed requests too. The plan must say this explicitly so an implementation agent does not accidentally branch around the persistence call.
...
### 2. Per-row result accumulator (array indexed by spreadsheet row order)
...
let currentRowIndex = -1; // tracks the active row for exception handling

for (let i = 0; i < rows.length; i += 1) {
  currentRowIndex = i;
  const row = rows[i];
  // ... per-row logic, updating queryResults[i] only in the branches
  // listed in the status mapping table below
}
```

`id` is a displayed value only; not used as a lookup key.

### 3. Status mapping (single source of truth) — V03

Each branch either **writes** to `queryResults[i]` or **leaves it alone and breaks**. Branches that fire after a real per-row outcome has already been written must NOT overwrite that row.

| Situation in current code (file:line approx)                                                                                              | Action on `queryResults[i]`                                       | Continue/break |
| ----------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- | -------------- |
| Pre-fetch abort check ([requestGoogleRssJob.ts:662-666](../worker-node/src/modules/jobs/requestGoogleRssJob.ts#L662))                     | write: `skipped / 0 / canceled`                                   | break          |
| `buildQuery` returns empty `query` ([requestGoogleRssJob.ts:669-672](../worker-node/src/modules/jobs/requestGoogleRssJob.ts#L669))        | write: `skipped / 0 / empty_query`                                | continue       |
| `wasRequestMadeRecently` returns true ([requestGoogleRssJob.ts:679-684](../worker-node/src/modules/jobs/requestGoogleRssJob.ts#L679))     | write: `skipped / 0 / repeat_window`                              | continue       |
| Post-fetch abort check ([requestGoogleRssJob.ts:693-697](../worker-node/src/modules/jobs/requestGoogleRssJob.ts#L693))                    | write: `skipped / 0 / canceled`                                   | break          |
| HTTP 503 — rate-limited ([requestGoogleRssJob.ts:699-704](../worker-node/src/modules/jobs/requestGoogleRssJob.ts#L699))                   | write: `failed / 0 / rate_limited`                                | break          |
| Non-503 RSS error ([requestGoogleRssJob.ts:691,706-716](../worker-node/src/modules/jobs/requestGoogleRssJob.ts#L691)) — **V03 clarified** | call `storeRequestAndArticles` first (preserves `NewsApiRequest` history); then write: `failed / 0 / rss_fetch_error: <msg>` | continue       |
| Success — `storeRequestAndArticles` returns `savedCount` ([requestGoogleRssJob.ts:706-717](../worker-node/src/modules/jobs/requestGoogleRssJob.ts#L706)) | write: `success / savedCount / null` (or `time_range_invalid`) | continue       |
| Target articles collected — early break ([requestGoogleRssJob.ts:719-727](../worker-node/src/modules/jobs/requestGoogleRssJob.ts#L719)) — **V03 fixed** | **DO NOT touch `queryResults[i]`** — the row already has its real `success` outcome from the line above | break |
| Post-delay abort check ([requestGoogleRssJob.ts:731-735](../worker-node/src/modules/jobs/requestGoogleRssJob.ts#L731)) — **V03 fixed**    | **DO NOT touch `queryResults[i]`** — the row already has its real outcome | break |
| Outer `try/catch` exception ([requestGoogleRssJob.ts:737-740](../worker-node/src/modules/jobs/requestGoogleRssJob.ts#L737))               | write: `queryResults[currentRowIndex] = failed / 0 / error: <msg>` (only if `currentRowIndex >= 0`) | (already failing) |
| Rows never reached (loop broke earlier; or rows after `currentRowIndex` when exception fires)                                             | leave at seeded `skipped / 0 / not_reached`                       | n/a            |

Three explicit rules emerge from the table:

- **Pre-work breaks (pre-fetch abort, post-fetch abort) write `canceled` for the active row, because nothing has been persisted for that row yet.**
- **Post-success breaks (target_reached, post-delay abort) do NOT write anything, because the row already has its real outcome.** The run-ending reason is already captured in `endingReason` / `endingMessage`.
- **Non-503 errors still call `storeRequestAndArticles`** (existing behavior — preserves the `NewsApiRequest` history row that `wasRequestMadeRecently` depends on).
...
`currentRowIndex` starts at `-1` so a failure before the loop begins (e.g., DB init throws) does not mislabel row 0.
...
1. `worker-node/src/modules/jobs/requestGoogleRssJob.ts`
   ...
   - Per the status mapping table:
     - Pre-fetch abort, post-fetch abort, empty query, repeat window, 503, non-503 RSS error, success — write `queryResults[i]` in place.
     - **Non-503 RSS error must still call `storeRequestAndArticles` with `status: response.status` and empty items before writing the failed row outcome** — preserves the existing `NewsApiRequest` history record that `wasRequestMadeRecently` depends on.
     - Target reached, post-delay abort — break the loop **without** modifying `queryResults[i]`.
   - In the outer `catch`, write `queryResults[currentRowIndex] = { ..., status: 'failed', note: 'error: <msg>' }` only when `currentRowIndex >= 0`.
...
3. `worker-node/tests/modules/jobs/requestGoogleRssJob.test.ts` (existing tests)
   ...
   - Add cases for each status branch: empty query, repeat window, non-503 fetch error, 503, success-with-saves, success-with-zero-saves, not_reached tail.
   - Add a test that throws inside `storeRequestAndArticles` mid-loop and asserts the active row is recorded as `failed / error: <msg>` while later rows stay `skipped / not_reached`.
   - Add a test where the spreadsheet has two rows with the same `id` and asserts both appear in the result array (no collision).
   - **V03:** add a test where `targetArticlesAddedCount` is met by the *successful* result of row N — assert row N remains `success / savedCount`, rows N+1.. remain `skipped / not_reached`, and `endingReason === 'target_articles_collected'`.
   - **V03:** add a test where cancellation fires at the post-delay checkpoint after a successful row — assert that row stays `success` and is not overwritten with `canceled`.
   - **V03:** add a test where `fetchRssItems` returns a non-503 error — assert `storeRequestAndArticles` is still invoked (mock spy) and the resulting `queryResults[i]` is `failed / 0 / rss_fetch_error: <msg>`.
...
- `storeRequestAndArticles` ([requestGoogleRssJob.ts:506](../worker-node/src/modules/jobs/requestGoogleRssJob.ts#L506)) — already returns `savedCount`. Use its return value directly to populate `saved_articles`. **Must continue to be called for non-503 RSS errors** to preserve the `NewsApiRequest` history row.
...
| Aborted runs leave the result partially populated          | Seed value (`skipped / not_reached`) handles unreached rows; pre/post-fetch aborts write `canceled` explicitly. |
| Duplicate `id` in the input spreadsheet                    | Result accumulator is an array indexed by row order; both rows appear in the report.                   |
| Exception fires mid-row                                    | `currentRowIndex` lets the outer `catch` mark the active row `failed / error: <msg>`.                  |
| **V03:** `target_reached` overwrites the row that succeeded | Status mapping rules explicitly forbid writing to `queryResults[i]` on `target_reached` or post-delay abort. |
| **V03:** Non-503 error branch could skip `storeRequestAndArticles` and silently drop the history record | Plan + tests require the call to remain in place; spy test asserts it is still invoked. |
...
2. `cd worker-node && npm test` — all existing tests pass; new tests cover each status branch, the duplicate-id case, the mid-row exception case, the target-reached-on-success case, the post-delay-cancel-after-success case, and the non-503-error-still-persists case.
...
5. Manually simulate a 503 (e.g., point at an unreachable host via env override in a test env) and confirm rows after the failing one are recorded as `skipped / not_reached`, and the failing row itself is `failed / rate_limited`.
