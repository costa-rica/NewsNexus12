...
- **v01**: initial cleanup plan (commit `cf5f531`, now superseded).
- **v02**: introduced the `set-state-in-effect` triage matrix, reframed
  TanStack Table as a policy decision, added CI gating section.
- **v03 (this doc)**: corrects two issues raised by a codex review of v02:
  - the warning-budget CI gate was described as preventing any new
    warning, but `--max-warnings=N` is a count ceiling, not an identity
    check. Described accurately below.
  - Pattern C (mount-time fetching) listed Next.js Server Components as
    the preferred fix. That is too ambitious for the current portal:
    these pages are `"use client"` and read a Redux Bearer token. Moving
    them to Server Components requires an auth/data-loading architecture
    change, not a lint cleanup. Pattern C now prefers a query hook /
    library or a scoped suppression by default.
...
is a `"use client"` page that reads `token` from Redux via
`useAppSelector` and calls the API with `Authorization: Bearer ${token}`.
On mount it calls `fetchBackupList()` and `fetchRowCountsByTable()`,
both of which ultimately call `setState`.

**Default fix options** (these are what a normal cleanup commit should
choose between):

1. **Adopt a data-fetching hook / library** (SWR or React Query). These
   have compiler-friendly hook shapes that satisfy the rule without an
   auth refactor. The hooks accept the existing `token` from Redux as
   input and handle loading, error, and refetch state.
2. **Keep the effect but justify with a scoped suppression**:
   `// eslint-disable-next-line react-hooks/set-state-in-effect` plus a
   one-line comment naming the constraint
   (e.g. "client-side auth; awaiting SWR migration").

Do **not** "fix" by inlining the `setState` into a `then(...)` callback —
the rule still fires on anything reachable from the effect body.

**Out of scope for this cleanup**: moving the fetch into a Next.js
Server Component or route loader. The portal pages currently use a
client-only Redux Bearer-token auth pattern. Migrating to Server
Component data loading is a separate architecture project that needs
its own scope: how server code reads the token (cookie vs. Redux
mirror), how authenticated state hydrates the client, and per-page QA.
A future PR that takes that on should include that scope, not be
folded into lint cleanup.
...
**errors** — it does nothing about warnings while cleanup is in flight.

There are two distinct things a CI gate could enforce. Pick deliberately:

### Option 1 — total warning budget (count ratchet)

Run lint with `--max-warnings=N` where `N` is the current count. CI
fails when the total exceeds `N`.

- **What this catches:** an absolute increase in warning count.
- **What this does NOT catch:** a change that fixes one warning and
  introduces a different unrelated warning in the same PR (count stays
  the same → CI passes). A change that replaces a warning at file X
  with a warning at file Y (count stays the same → CI passes).
- **When to use:** as a coarse ratchet during active cleanup. Lower
  `N` with each cleanup commit so progress is one-way.
- **Initial value:** `--max-warnings=52`.

### Option 2 — warning identity baseline

Snapshot the current 52 warnings as a baseline file (each entry keyed
by `file:line:rule`, optionally `message`). CI runs lint, diffs the
current warning set against the baseline, and fails on any entry that
is not in the baseline.

- **What this catches:** any new warning, even if the count stays the
  same.
- **What this does NOT catch:** an existing warning that legitimately
  moved when nearby lines were edited (will look like a new warning
  unless `line` is excluded from the key). Tolerable if the cleanup
  commit's PR description acknowledges line drift.
- **When to use:** when "no new warnings of any kind" is the goal.
- **Implementation note:** there is no built-in ESLint flag for this;
  it needs a short script (`eslint --format=json | jq | diff`) or a
  package like `eslint-baseline`.

### Option 3 — strict (zero warnings)

`npm run lint --workspace newsnexus12portal -- --max-warnings=0`. Only
passes when there are no warnings at all. Stricter; blocks all
in-progress cleanup until each rule is fully retired.

### Recommendation

**During cleanup:** Option 1 with `--max-warnings=52`, lowered per
commit. Combine with PR-review discipline that asks "did you check for
new warnings that happen to keep the count the same?" — until Option 2
is implemented, this is the human check that closes the gap.

**After cleanup completes:** flip the three downgraded rules back to
`error` in `portal/eslint.config.mjs` and run plain
`npm run lint --workspace newsnexus12portal` in CI. At that point the
budget flag is unnecessary.

If "no new warnings" enforcement is a hard requirement before cleanup
finishes, implement Option 2 as a small dedicated PR. Do not rely on
Option 1 to deliver Option 2's guarantee.
