...
# worker-python codex cli ai approver todo v01
...
## Phase 1 - Configuration selection and test bootstrap
...
- [ ] Update `worker-python/tests/conftest.py` before changing startup validation so ordinary test collection stays deterministic:
  - [ ] Add `os.environ.setdefault("USE_OPEN_AI_API", "true")` before `from src.main import app`.
...
Include a body noting the test bootstrap contract and the Codex-default selection semantics.
...
- [ ] Add or update route tests if needed:
  ...
  - [ ] Keep route tests on the OpenAI API backend through the `tests/conftest.py` bootstrap unless a test explicitly isolates and monkeypatches the Codex path.
