---
created_at: 2026-09-04T16:55:22Z
updated_at: 2026-09-04T16:55:22Z
created_by: codex (gpt-5.6-sol) nicksmacbookair
modified_by: codex (gpt-5.6-sol) nicksmacbookair
---

# Weekly Article Processing Cron PRD V06

## Status

- Status: revised requirements amendment
- Base requirements: `docs/20260829_weekly_article_processing_cron_prd_v03.md`
- Supersedes: `docs/20260904_weekly_article_processing_cron_prd_v05.md`
- Assessments incorporated:
  - `docs/20260904_weekly_article_processing_cron_prd_v04_assessment_claude.md`
  - `docs/20260904_weekly_article_processing_cron_prd_v05_assessment_claude.md`
...
This V06 amendment supersedes only the conflicting host allowlist, database allowlist, development database confirmation, production database-role, and rollout-validation requirements in V03.
...
### 2.4 Shared Production Identity Check

The operator accepts the assessment recommendation to define the production identity rules once and reuse them.

Normal preflight and `run-config-check` must call the same production identity function. This prevents the rollout validation command from approving an identity that normal execution rejects.

This shared function does not restore any removed hostname or database-name safeguard. It only keeps the accepted `limited_user` and `newsnexus_app` requirements consistent.
...
Add a comment to `ops/weekly-article-flow/.env.example` stating that production weekly-flow execution expects `PG_USER=newsnexus_app`. Keep the example free of credentials and do not change another package's example file.
...
Export these checks from one shared weekly-flow module. Both normal preflight and `run-config-check` must call that same function and return the same error for the same invalid identity.

Do not duplicate the account names, role names, conditions, or error messages in the two entry points.
...
- call `sequelize.sync()` or any schema-changing operation
...
8. Export one shared production identity function for preflight and `run-config-check`.
9. Add the defined `run-config-check` wrapper and implementation.
10. Add `run-config-check` to `install.sh` source checks and shell syntax validation.
11. Add `run-config-check` to the wrapper test's executable, argument-forwarding, and no-scheduler checks.
12. Preserve automatic host and database evidence.
13. Preserve resume-context matching.
14. Update all affected test helpers and fixtures.
15. Update `.env.example`, README, development runbook, and production activation runbook.
16. Keep implementation changes inside `ops/weekly-article-flow` and its runbooks.
...
- Preflight and `run-config-check` call the same identity function.
- Both surfaces reject the same invalid identity with the same message.
...
- The wrapper is executable and forwards `"$@"` safely.
- `install.sh --check` syntax-checks the wrapper.
- The wrapper contains no systemd, timer, or cron control.
...
4. Remove obsolete allowlist entries from weekly-flow environment files as cleanup.
5. Remove `--confirm-dev-database` from saved development commands.
6. Run schema validation with the one-command `newsnexus_boot` override.
7. Run `run-config-check` through the protected `runuser` invocation.
8. Confirm worker queues are idle and no weekly run is active.
9. Complete one supervised `manual_production` run as required by V03.
10. Review PostgreSQL, backup, worker, JSONL, journald, and alert evidence.
11. Obtain operator approval.
12. Enable the production timer.
13. Confirm its next Friday 5:00 AM Pacific trigger.
...
9. Preflight and `run-config-check` use one shared production identity function.
10. Both identity-check surfaces reject the same invalid value with the same message.
11. `run-config-check` satisfies every boundary in section 12.
12. Installer and wrapper tests cover `run-config-check` as an executable shell asset.
13. Hostname and database name remain recorded as run evidence.
14. Resume-context validation remains intact.
15. All unrelated V03 preflight and recovery safeguards remain intact.
16. `.env.example` has no obsolete allowlist and documents the expected production `PG_USER` in a comment.
17. Development documentation contains no removed variables or argument.
18. Production documentation restores the supervised manual-production gate.
19. Every weekly-flow environment edit occurs while the production timer is disabled.
20. All affected tests, builds, integration checks, and operational checks pass.
21. Production scheduling remains disabled until the supervised production run succeeds and the operator approves activation.
