...
# Weekly continuation implementation TODO v01
...
- [ ] Add a cheap list-level continuation signal to recent runs, or expose a cheap batched endpoint.
...
- [ ] Reject missing, running, completed, `completed_no_new_articles`, pre-Google-RSS, already-active-continuation, and unrecognized failure shapes.
...
- [ ] Ensure active-run guard treats standard and continuation runs the same.
- [ ] Preserve normal weekly run behavior as the default path.
- [ ] For downstream continuation after Google RSS, use the source `articleIdMinExclusive`.
- [ ] Capture current global max article id when continuation reaches downstream processing.
...
- [ ] Preserve worker-node response bodies and status codes.
- [ ] Explicitly verify preservation for:
  - [ ] `200` assessment responses
  - [ ] `202` accepted continuation starts
  - [ ] `404` missing source run responses
  - [ ] `409` active-run or no-longer-eligible responses
  - [ ] `422` recognized but ineligible or unsupported run shape responses if worker-node uses that style
...
- [ ] Surface no-longer-eligible and unsupported-shape responses clearly.
