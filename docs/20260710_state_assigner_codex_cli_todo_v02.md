...
# State Assigner Codex CLI Backend Todo (v02)
...
Addresses all three concerns from `20260710_state_assigner_codex_cli_todo_v01_assessment_codex.md`:

1. **Phase sequencing** — Phase 2 now explicitly *extracts copies* into the new modules and leaves `src/modules/jobs/stateAssignerJob.ts` untouched; the job-side deletion and rewiring happen only in Phase 4. Every phase compiles, tests, and builds green on its own. A shared prompt-building helper (`prompt.ts`) is added so neither client duplicates the template-replacement logic.
2. **Route test seam** — Phase 5 extends `StateAssignerRouteDependencies` with `resolveAiConfig?: typeof resolveStateAssignerAiConfig` (production default: the real resolver with the real binary check) so route tests inject a deterministic resolver instead of module-mocking or requiring `codex` on `PATH`.
3. **stdin stream errors** — Phase 3 adds explicit `child.stdin` error handling (attach an `error` listener before writing, handle write/end callback errors, treat `EPIPE`-style early close as a bounded per-article failure) plus a test where the fake child closes stdin early.
...
- [ ] In `runLegacyWorkflow`: select the analyzer by `aiConfig.backend` (`analyzeArticleWithOpenAi` from `openAiClient.ts` vs `analyzeArticleWithCodexCli` from `codexCliClient.ts`) and select `iterationTimeoutMs` by backend: `DEFAULT_ITERATION_TIMEOUT_MS` (10 s) for `openai`, `aiConfig.codexTimeoutMs` for `codex-cli`.
