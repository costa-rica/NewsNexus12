---
created_at: 2026-08-29T21:41:26Z
updated_at: 2026-08-29T21:41:26Z
created_by: codex (gpt-5.6) nicksmacbookair
modified_by: codex (gpt-5.6) nicksmacbookair
---

# Legacy Orchestrator and AI Approver V01 Removal Plan V01
...
1. AI Approver V01, including its scoring runtime, prompt management, article-review integration, models, and operator material.
...
AI Approver V02 and independently queued jobs remain in place. Retained Python files named `orchestrator.py` describe internal single-workflow classes and are outside the deletion boundary.
...
Refactor the article review page and `TableReviewArticles.tsx` around the V02 prediction data only:
...
- Remove the V01 score column, sorting helper, click callback, cell status rendering, and modal launch.
- Remove `ModalAiApproverDetails.tsx` and the V01 one-off review modal path. `ModalReviewArticleContent.tsx` is also deleted because its API dependency is the V01 `/analysis/ai-approver/review-article-content/:articleId` route.
- Remove V01 properties and response interfaces from `types/article.ts` while preserving all `AiApprover*V02` types.
- Preserve V02 table filtering, details, human validation, comments, and prediction refresh behavior.

The portal result is a smaller review-data pipeline: article retrieval and V02 prediction retrieval remain independent, then V02 predictions are merged into displayed rows.
...
- Mount the retained queue-backed starter routes directly. Their existing global queue with concurrency one remains the only execution coordination mechanism.
...
Rewrite `scripts/README.md` around the retained scripts. Do not archive executable service, trigger, or schema helpers because an archived copy could still be mistaken for an installation source.
...
Historical documents that merely describe prior behavior may keep the old terminology after archiving. Active documentation may use `orchestrator` only for the retained internal Python classes or when explaining that the removed product feature no longer exists.
...
3. Test and build API, including retained proxy regressions and removed-route `404` coverage.
...
6. Run portal lint and build, then manually inspect the automations and article review pages.
7. Search live source, scripts, and active documentation for the removed route, header, model, body contract, and product-name usages. Remaining matches must be retained Python workflow classes, archived history, or current removal records.

After the repository suites pass, the operator creates or confirms an unchanged pre-removal ZIP from code that still knows the legacy models. The removal branch then imports that ZIP, allowing db-manager to rebuild `public` from retained models and skip legacy CSVs.

The post-import smoke test starts each service without V01 environment settings and without the four tables or removed column. It exercises standalone Google RSS, scraper, state assigner, semantic scorer, location scorer, deduper, and the V02 preview-to-review path. Removed portal and HTTP paths must produce ordinary `404` responses.
...
Implementation is complete when live code cannot start, query, configure, review, continue, or report either legacy feature; old backups restore retained data without rebuilding legacy schema; all retained workflows pass their regression paths; repository assets cannot reinstall the old schedule; and operator evidence shows no pre-existing NewsNexus12 Ubuntu schedule remains enabled.
