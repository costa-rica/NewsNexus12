...
# Review Page Chunk Pagination — Todo v01
...
- [ ] `portal/src/app/(dashboard)/articles/review/page.tsx` — fetch flow:
  - new local state: `chunkStartCursors: (number | null)[]` (init `[null]`),
    `currentChunkIndex: number` (init 0), `totalCount: number | null`, `hasMore: boolean`;
  ...
  - on response: `setArticlesArray` with the chunk; store `hasMore`; store `totalCount` only
    when it is non-null (first chunk);
...
- [ ] Review page — chunk navigation handlers:
  - Next: `fetchArticlesArray(nextCursor)`, push `nextCursor` onto `chunkStartCursors`,
    increment `currentChunkIndex`; enabled only when `hasMore`;
  - Prev: `fetchArticlesArray(chunkStartCursors[currentChunkIndex - 1])`, decrement index;
    enabled only when `currentChunkIndex > 0`;
  - filter change / "Refresh with New Filters" / limit change: reset stack to `[null]`, index
    to 0, then fetch chunk 1.
- [ ] Review page — `hasFilterChanges` (~lines 93–107) and `initialFilters` (~lines 82–91):
  include `limit` so changing the dropdown lights up the Refresh button like other filters.
- [ ] Filter Articles card (~lines 849–959):
  - limit dropdown using `portal/src/components/form/Select.tsx`, options 1,000 / 5,000 /
    10,000 / 20,000, value from `articleTableBodyParams.limit ?? 5000`, dispatching
    `updateArticleTableBodyParams({ limit })`;
  - total-count display from `totalCount` (e.g. `12,483 articles match the current filters`;
    hide or show placeholder while null);
  - `Chunk n of m` indicator (`m = Math.ceil(totalCount / limit)`; only when `totalCount`
    known) plus Prev/Next buttons styled like the existing toggle buttons.
...
- [ ] Manual end-to-end against a dev database (api + portal dev servers per `AGENTS.md`):
  ...
  - change limit to 1,000, Refresh lights up, refetch shows `Chunk 1 of m` with recomputed m;
  - Next/Prev traverse chunks with no duplicated or skipped articles at chunk boundaries
    (check boundary ids);
  - toggling Hide Approved / Hide Irrelevant refetches chunk 1 and changes `totalCount`;
