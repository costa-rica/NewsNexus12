...
- Update the latest run reference, model name, status, prediction, reasoning, and errors.
...
Track two counters during a run:

1. consecutive final Codex CLI failures
2. consecutive `invalid_response` outcomes

Rules:

- Stop after three consecutive Codex CLI failures.
- Stop after five consecutive `invalid_response` outcomes.
...
Suggested worker-python namespace:
