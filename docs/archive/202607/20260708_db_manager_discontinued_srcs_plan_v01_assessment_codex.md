...
# Db manager discontinued sources plan v01 assessment

The plan is close to implementable, and its main architecture matches the existing `deleteArticlesNoState` module. I found two concerns that should be resolved in a v02 before creating the todo, because both affect whether an implementer can safely verify a destructive source-based deletion.
...
This is a real implementation risk because the feature is destructive and the core safety property is the candidate query. The v02 plan should require one of these verification strategies:

1. Add SQL-shape assertions in the module tests that inspect the query passed to `sequelize.query` and require:
...
2. Or add a small database-backed query test fixture that creates source rows, finder rows, articles, and protection rows, then runs the preview query against real tables.
...
1. Strengthen the testing plan so candidate-selection correctness is verified through SQL-shape assertions or a real-query fixture, not only mocked `sequelize.query` return rows.
