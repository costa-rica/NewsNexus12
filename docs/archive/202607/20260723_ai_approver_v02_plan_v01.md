...
# AI Approver V02 Implementation Plan V01
...
Store only article IDs and selection metadata in the snapshot. Do not duplicate article content or rendered prompts.
...
- call `sync` only on the three V02 models in dependency order
- use no `force` or `alter` option
- create only the three V02 tables, indexes, and foreign keys
- detect already-existing compatible tables
- fail on incompatible partial schema
...
- create a draft run
- store the resolved IDs and options
...
Expired drafts cannot be started. Preview and start paths mark expired drafts `failed` with `endingReason = preview_expired` and an end timestamp.
...
- article hydration from frozen IDs
...
2. load the frozen selection data or current article data required by the snapshot contract
...
Extend the existing `/automations` router with authenticated V02 routes for:
...
Proxy preview and start to worker-python. Reuse common worker connection and Axios error forwarding.
