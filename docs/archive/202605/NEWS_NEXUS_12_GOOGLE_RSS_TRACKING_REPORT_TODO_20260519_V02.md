...
# News Nexus 12 Google RSS Tracking Report TODO — V02
...
Supersedes: [NEWS_NEXUS_12_GOOGLE_RSS_TRACKING_REPORT_TODO_20260519.md](NEWS_NEXUS_12_GOOGLE_RSS_TRACKING_REPORT_TODO_20260519.md)

## Changes from V01

V01 was reviewed by Codex ([assessment](NEWS_NEXUS_12_GOOGLE_RSS_TRACKING_REPORT_TODO_20260519_assessment_codex.md)). Two concerns, both accepted:

1. **V01 asked for spies on `wasRequestMadeRecently` and `storeRequestAndArticles`** — both are module-private and not injectable. V02 commits to a **public-boundary testing strategy**: mock `global.fetch` and assert against DB state via the Sequelize models. No new exports, no DI refactor.
2. **V01's non-503 error test described the wrong mock shape** — it passed `fetchRssItems`'s internal return shape into a `global.fetch` mock. V02 corrects this to a Response-like object (`{ ok: false, status: 500, text: () => '...' }`).

The implementation work itself (Phases 1, 2, 3, 4) is unchanged. Only the test instructions in §2.4 changed.
...
### 2.4 Tests — public-boundary only (V02)
...
- [ ] **Empty query row** — spreadsheet has one row with all keyword/phrase columns blank. Assert `queryResults[i].status === 'skipped'`, `note === 'empty_query'`, and no `NewsApiRequest` row was created for that row.
...
- [ ] **Mid-row exception (V02 corrected approach)** — `jest.spyOn(Article, 'create').mockImplementationOnce(...)` to throw on the first call inside row N's processing. `global.fetch` mock returns a valid 1-item payload first. Assert: row N is `failed / 0 / error: <msg>`; rows after stay `skipped / not_reached`; rows before keep their real outcomes; `endingReason === 'error'`.
...
- **V02:** Adding new exports or a dependency-injection seam to `requestGoogleRssJob.ts` for the sake of testing. Use `global.fetch` mocks and DB-state assertions instead. If a branch genuinely can't be exercised that way, stop and ask.
